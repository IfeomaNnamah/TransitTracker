...
const PdfGenerator = (props) => {
  ...
  const generatePdf = () => {
    ...
    html3pdf(pdfContent, {
      margin: 10,
      filename: `PO-${selectedPackingListPackages.packages ? selectedPackingListPackages?.packages[0]?.purchaseOrderItemSupplies[0]?.purchaseOrderItem?.purchaseOrderNumber : ""}.pdf`,
        ...
        <table className='custom-table custom mt-2' style={{tableLayout: 'fixed', width: '100%',}}>
          <thead>
            <tr>
              ...
              <th className='text-center' style={{ width: '5%' }}>Quantity</th>
            </tr>
          </thead>
          <tbody>
            {
                      ...
                      {/* <td style={columnStyle("center")}>
                          <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                              {pck.purchaseOrderItemSupplies
                                  .map((item, idx) => (
                                      <div key={idx} style={{padding: "5px", minHeight: "35px"}}>
