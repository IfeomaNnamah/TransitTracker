...
import { POItemSupplyStatus, POItemStatus, formatCurrency, formatDateTime, getCurrencySymbol, getPurchaseOrderItemStatus, getPOItemSupplyStatusIndex } from "../../helpers";
...
const PurchaseOrder =  () => {
    ...
    const pageContext = useSelector((state: any) => state.pageContext.value);
    ...
    useEffect(() => {
        if(accessToken && purchaseOrderId) getPurchaseOrderItems(purchaseOrderId) // eslint-disable-next-line
    ...
    useEffect(() => {
        if(accessToken && purchaseOrderNumber) {
            console.log("here")
            getPurchaseOrder()
            navigate("/transitofficer/purchaseorders/"+purchaseOrderNumber)
    ...
    const page = "Purchase Orders Tracking"
        ...
        <Layout title={page}>
            <div className="container">
                <div className="main">
                    ...
                    {pageContext?.from && <div className="main-inner mt-2">                  
                        <div className="detail-top-section">
                            <Link to="/transitofficer/purchaseorderassignmenttofreightforwader" state={{status: statusAfterNavigation?.status}} className="actions">
                                <p><span className="material-symbols-rounded">arrow_back</span> Back to Purchase Order Assignment</p>
                            </Link>
                        </div> 
                    </div> }
