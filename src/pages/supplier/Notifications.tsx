...
const Notifications =  () => {
    ...
    const getMaterialReadinessDocumentNotifications = (isLoading:boolean) => {
        setIsLoading(isLoading)
        var request: Record<string, any> = {
            what: "getMaterialReadinessDocumentNotifications",
            params: {
                Page: currentPage,
    ...
    const markNotificationsAsRead = (commentIds: any) => {
        var request: Record<string, any> = {
            what: "markMaterialReadinessDocumentNotificationsAsRead",
            data: {
                materialReadinessDocumentCommentIds: commentIds
            }
        };
        
        makePatchRequest(request)
            .then(() => {                
                getMaterialReadinessDocumentNotifications(false) // Don't show loader when called from here.
            })
            .catch((error:any) => 
    ...
    const getUnreadMaterialReadinessDocumentNotifications = () => {
        var request: Record<string, any> = {
            what: "getUnreadMaterialReadinessDocumentNotifications",
            params: {
                Receiver: user?.id,
            }
        };
            ...
            <div className="notification-card" key={i}>
                ...
                <div>
                    <div className="notification-title">
                        ...
                        <span className='link-text' onClick={() => navigate("/supplier/view/materialreadinessdocument/"+data.materialReadinessDocumentId)}>Go to Page</span>
                    </div>
                    
                    {/* <p>{data.message}</p>    */}
                    {
                        data.message.split('|').map((message: string, index: number) => {
    ...
    useEffect(() => {
        if(accessToken) {
            getMaterialReadinessDocumentNotifications(true) // show loader when called from here.
            getUnreadMaterialReadinessDocumentNotifications()
        }
        // eslint-disable-next-line
    }, [accessToken, currentPage, itemsPerPage, searchValue, dateRange])
