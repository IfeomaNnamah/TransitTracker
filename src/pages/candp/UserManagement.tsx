...
import { entityDepartments, handleCopy } from 'helpers';
...
const UserManagement =  () => {
    ...
    const ThirdPartyRoles = ["Supplier", ]
    ...
    const getUser = (id:string) => {
        ...
        setFormData({
            ...
            entityDepartment: user?.user.department?.split(";")[2] ? user?.user.department?.split(";")[2]?.trim() : "", 
        })
    ...
    const HandleOnboardUser = () => {     
        ...
        if(isCompanyNameSet && isAddressSet) {  
            ...
            if (Object.values(data).every(value => value)){
                ...
                data.department = `${formData.department}; ${formData.entityDepartment}`
                var request:Record<string, any> = {
                    what: "OnboardUser",
                    data: data
                };             
        ...
        <Layout title={page}>
            ...
            <Modal isOpen={createUserModal} style={customStyles} className="modal modal-5" ariaHideApp={false}>
                ...
                <div className="modal-body" style={{maxHeight: "400px", overflowY: "auto"}}>
                    ...
                    {selectedValue?.includes("Entity Representative") && <>
                        <label><span className="errorX mr-2">*</span>Entity Department</label>
                        <select
                        onChange={(e) => setFormData({...formData, 'entityDepartment': e.target.value})}
                        name="entityDepartment" 
                        value={formData.entityDepartment}
                        disabled={!formData.roleIds.length}
                        required={selectedValue?.includes("Entity Representative")}
                        >
                            <option value="" key="0" disabled>Select...</option>
                            {
                                entityDepartments?.map((department: any, index: any) => (
                                    <option value={department} key={index}>{department}</option>
                                ))
                            }
                        </select>
                        <p className="error">{ errorData.entityDepartment }</p>
                    </>}                           
                </div>
            ...
            <Modal isOpen={updateUserModal} style={customStyles} className="modal modal-5" ariaHideApp={false}>
                ...
                <div className="modal-footer bt-1">
                    ...
                    <button disabled={isSubmitting} type="submit" className="custom-button orange"
                        onClick={() => HandleEditUser()}>{isSubmitting ? "Loading..." : "Submit"}</button>
                </div>
            </Modal>
