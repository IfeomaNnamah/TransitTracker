...
const ProofOfCollection =  () => {
    ...
    const handleSearch = (event: any) => {
        ...
        if(!value) {setSearchValue(""); setStatus(2)}
    }

    const [status, setStatus] = useState<number | null>(statusBeforeNavigation?.status ? statusBeforeNavigation.status : 2)
    const getProofOfCollection = () => {
        setIsLoading(true)
        var request: Record<string, any> = {
            what: "getProofOfCollectionForCAndP",
            params: {
                Page: currentPage,
    ...
    const row = proofOfCollection.map((data, index) => {
                    return (
                        <tr key={index}>
                            ...
                            <td>
                                <button onClick={() => navigate("/candp/proofofcollection/"+data.id, {state: {status: status}})} className="actions">
